...
struct trapframe_t {
	...
	u32 ecx;	
	u32 eax;
	u32 trapno; /* trap number pushed by isr.asm */
	u32 errcode; /* either error code or dummy byte pushed by isr.asm */
...
struct pte_t { /* this structure is used both for PTEs and PDEs */
	union {
		...
		struct {
			...
			u32 dirty : 1; /* i don't understand the difference with ^ */
...
/* returns the address part of a pte */
#define PTE_ADDR(pte)   ((pte.as_u32) & ~0xFFF)
/* returns the flags part of a pte */
#define PTE_FLAGS(pte)  ((pte.as_u32) &  0xFFF)

#define PTE_EMPTY(pte) (pte.as_u32 == 0)

#define PTE_PRESENT		(0x1)
#define PTE_RW			(0x1 << 1)
#define PTE_USR			(0x1 << 2)

#define VADDR_PDE(virt) (((u32)virt >> 22) & 0x3ff)
#define VADDR_PTE(virt) (((u32)virt >> 12) & 0x3ff)
#define VADDR_OFFSET(virt) (((u32)virt & 0xfff)) 
#define VADDR(pd, pt, offset) ((pd << 22) | (pt << 12) | offset)
